...
def make_client(tmp_path: Path, token: str = "a" * 32) -> TestClient:
    app = FastAPI()
    app.state.settings = SimpleNamespace(QWENDBC_ACCESS_TOKEN=token)
...
def test_cowork_api_requires_bearer_and_records_timeline(tmp_path: Path):
    ...
    created = client.post("/api/v1/conversations", json={"title": "Case"}, headers=headers)
...
def test_different_bearer_principal_cannot_access_conversation(tmp_path: Path):
    client = make_client(tmp_path, token="")
    first = {"Authorization": "Bearer principal-one"}
    second = {"Authorization": "Bearer principal-two"}
    created = client.post("/api/v1/conversations", json={"title": "Private"}, headers=first)
    conversation_id = created.json()["id"]
    response = client.get(f"/api/v1/conversations/{conversation_id}", headers=second)
